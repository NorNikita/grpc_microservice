...
func TestACL(t *testing.T) {
	...
	biz := contract.NewBizClient(conn)
	adm := contract.NewAdminClient(conn)
	...
	for idx, ctx := range []context.Context{
		...
		_, err = biz.Test(ctx, &contract.Nothing{})
		if err == nil {
	...
	// есть доступ
	_, err = biz.Check(getConsumerCtx("biz_user"), &contract.Nothing{})
	if err != nil {
		t.Fatalf("ACL fail: unexpected error: %v", err)
	}
	_, err = biz.Check(getConsumerCtx("biz_admin"), &contract.Nothing{})
	if err != nil {
		t.Fatalf("ACL fail: unexpected error: %v", err)
	}
	_, err = biz.Test(getConsumerCtx("biz_admin"), &contract.Nothing{})
	if err != nil {
		t.Fatalf("ACL fail: unexpected error: %v", err)
	}

	// ACL на методах, которые возвращают поток данных
	logger, err := adm.Logging(getConsumerCtx("unknown"), &contract.Nothing{})
	_, err = logger.Recv()
	if err == nil {
...
func TestLogging(t *testing.T) {
	...
	logStream1, err := adm.Logging(getConsumerCtx("logger"), &contract.Nothing{})
	time.Sleep(1 * time.Millisecond)

	logStream2, err := adm.Logging(getConsumerCtx("logger"), &contract.Nothing{})

	logData1 := []*contract.Event{}
	logData2 := []*contract.Event{}
	...
	go func() {
		...
		for i := 0; i < 4; i++ {
			...
			logData1 = append(logData1, &contract.Event{Consumer: evt.Consumer, Method: evt.Method})
		}
	}()
	go func() {
		defer wg.Done()
		for i := 0; i < 3; i++ {
			...
			// protobuf добавляет к структуре свои поля, которвые не видны при приведении к строке и при reflect.DeepEqual
			// поэтому берем не оригинал сообщения, а только нужные значения
			logData2 = append(logData2, &contract.Event{Consumer: evt.Consumer, Method: evt.Method})
		}
	}()

	biz.Check(getConsumerCtx("biz_user"), &contract.Nothing{})
	time.Sleep(2 * time.Millisecond)

	biz.Check(getConsumerCtx("biz_admin"), &contract.Nothing{})
	time.Sleep(2 * time.Millisecond)

	biz.Test(getConsumerCtx("biz_admin"), &contract.Nothing{})
	time.Sleep(2 * time.Millisecond)

	wg.Wait()

	expectedLogData1 := []*contract.Event{
	...
	expectedLogData2 := []*contract.Event{
		{Consumer: "biz_user", Method: "/main.Biz/Check"},
		{Consumer: "biz_admin", Method: "/main.Biz/Check"},
		{Consumer: "biz_admin", Method: "/main.Biz/Test"},
	}
...
func TestStat(t *testing.T) {
	...
	statStream1, err := adm.Statistics(getConsumerCtx("stat"), &contract.StatInterval{IntervalSeconds: 2})
	wait(1)
	statStream2, err := adm.Statistics(getConsumerCtx("stat"), &contract.StatInterval{IntervalSeconds: 3})
	...
	biz.Check(getConsumerCtx("biz_user"), &contract.Nothing{})
	biz.Add(getConsumerCtx("biz_user"), &contract.Nothing{})
	biz.Test(getConsumerCtx("biz_admin"), &contract.Nothing{})

	wait(200) // 2 sec

	expectedStat1 := &contract.Stat{
	...
	biz.Add(getConsumerCtx("biz_admin"), &contract.Nothing{})
